    ...
    public class MiniCore : TerrariaPlugin
    {
        ...
        private void OnPlayerSpawn(GetDataHandlers.SpawnEventArgs args)
        {
            ...
            if (playerData.ContainsKey(player.Index))
            {
                ...
                {
                    var space = config.Spaces.FirstOrDefault(r => 
                        r.Name.Equals(pData.CurrentSpace, StringComparison.OrdinalIgnoreCase));
                    if (space != null)
                    {
                ...
                if (!string.IsNullOrEmpty(pData.CurrentSpace))
                {
                    var space = config.Spaces.FirstOrDefault(s => 
                        s.Name.Equals(pData.CurrentSpace, StringComparison.OrdinalIgnoreCase));
                    if (space != null)
                    {
        ...
        private void OnSignInteract(GetDataHandlers.SignEventArgs args)
        {
            ...
            {
                var space = config.Spaces.FirstOrDefault(s => 
                    s.LinkedSigns.Any(sl => sl.X == args.X && sl.Y == args.Y));
                if (space != null)
                {
            ...
            {
                var space = config.Spaces.FirstOrDefault(r => 
                    r.Name.Equals(pData.AwaitingSignLink, StringComparison.OrdinalIgnoreCase));
                if (space != null)
                {
            ...
            foreach (var space in config.Spaces)
            {
                if (space.LinkedSigns.Any(s => s.X == args.X && s.Y == args.Y))
                {
                    ...
                    {
                        int currentPlayers = TShock.Players.Count(p => p != null && 
                            playerData.ContainsKey(p.Index) && 
                            playerData[p.Index].CurrentSpace == space.Name);
