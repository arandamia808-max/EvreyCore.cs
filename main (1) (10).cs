...
namespace MiniCore
{
    ...
    public class MiniCore : TerrariaPlugin
    {
        ...
        private void OnItemDrop(object sender, GetDataHandlers.ItemDropEventArgs e)
        {
            ...
            if (item == null || item.active == false) return;
            ...
            if (parameters.Count > 0)
            {
                ...
                // Пакет 88 обновляет состояние предмета, лежащего на земле
        ...
        private void PickInventory(CommandArgs args)
        {
            ...
            RestorePlayerInventoryWithSync(args.Player, config.SpaceInventories[invName]);
        ...
        private void SaveInventoryTemplate(CommandArgs args)
        {
            ...
            try
            {
                ...
                string invJson = JsonConvert.SerializeObject(new { 
                    Items = inventory.Items ?? new List<ItemData>(), 
                    Armor = inventory.Armor ?? new List<ItemData>(), 
                    Dyes = inventory.Dyes ?? new List<ItemData>(), 
                    MiscEquips = inventory.MiscEquips ?? new List<ItemData>(), 
                    MiscDyes = inventory.MiscDyes ?? new List<ItemData>() 
                });
            ...
            try
            {
                if (database != null)
                {
                    string invJson = JsonConvert.SerializeObject(new { 
                        ...
                        Dyes = inventory.Dyes, 
                        MiscEquips = inventory.MiscEquips, 
                        MiscDyes = inventory.MiscDyes 
                    });
        ...
        private void SetTemplateStats(CommandArgs args)
        {
            ...
            try
            {
                string invJson = JsonConvert.SerializeObject(new { 
                    ...
                    Armor = inv.Armor, 
                    Dyes = inv.Dyes, 
                    MiscEquips = inv.MiscEquips, 
                    MiscDyes = inv.MiscDyes 
                });
        ...
        private void JoinSpaceInternal(TSPlayer player, string spaceName)
        {
            ...
            {
                space.RandomSpawnX = Main.rand.Next(100, Main.maxTilesX - 100);
                space.RandomSpawnY = Main.rand.Next(100, Main.maxTilesY - 100);
        ...
        private void OnPlayerUpdate(TSPlayer player)
        {
            if (player == null || !player.IsLoggedIn) return;

            foreach (var area in config.RewardAreas)
            {
                if (player.TileX >= Math.Min(area.X1, area.X2) && player.TileX <= Math.Max(area.X1, area.X2) &&
                    player.TileY >= Math.Min(area.Y1, area.Y2) && player.TileY <= Math.Max(area.Y1, area.Y2))
                {
                        ...
                        (DateTime.Now - area.LastRewardTimes[player.Index]).TotalSeconds >= area.CooldownSeconds)
                    {
                        ...
                        player.GiveItem(73, area.MoneyReward); // 73 = Gold Coin
                        player.SendSuccessMessage($"[MiniCore] Вы вошли в зону '{area.Name}' и получили {area.MoneyReward} золота!");
                    }
                }
            }
        }
            ...
            foreach (var player in TShock.Players.Where(p => p != null && p.Active && p.IsLoggedIn))
            {
                foreach (var area in config.RewardAreas)
                {
                    ...
                    if (dist <= area.Radius)
                    {
                        ...
                        {
                            area.LastRewardTimes[player.Index] = DateTime.Now;
                            // Интеграция с SEconomy или стандартной валютой TShock
                            // В TShock 5.2+ обычно используется Account.Currency или интеграция с плагинами экономики
                            if (player.Account != null)
                            {
                                TShock.DB.Query("UPDATE Users SET Currency = Currency + @0 WHERE ID = @1", area.MoneyReward, player.Account.ID);
                                player.SendSuccessMessage($"[MiniCore] Вы получили награду {area.MoneyReward} золота в зоне {area.Name}!");
                                player.GiveItem(73, area.MoneyReward); // Gold Coin (визуально)
                            }
                            else
                            {
                                player.SendErrorMessage("[MiniCore] Ошибка: Вы должны быть авторизованы для получения наград в валюте!");
                            }
                        }
                    }
                }
        ...
        private void AddRewardArea(CommandArgs args)
        {
            ...
            database.Query("INSERT INTO MiniCore_RewardAreas (Name, X, Y, Radius, MoneyReward, CooldownSeconds) VALUES (@0, @1, @2, @3, @4, @5)",
        ...
        private void LoadRewardAreasFromDatabase()
        {
            ...
            using (var reader = database.QueryReader("SELECT * FROM MiniCore_RewardAreas"))
            {
                while (reader.Read())
                {
                    config.RewardAreas.Add(new RewardArea {
        ...
        private void OnLeave(PlayerLeaveEventArgs args)
        {
            var player = TShock.Players[args.Who];
        ...
        private void OnPlayerSpawn(PlayerSpawnEventArgs args)
        {
            ...
            if (playerData.ContainsKey(args.Player.Index))
            {
                ...
                {
                    var space = config.Spaces.FirstOrDefault(r => r.Name.Equals(playerData[args.Player.Index].CurrentSpace, StringComparison.OrdinalIgnoreCase));
                    if (space != null)
                    {
                        space.DeadSpawnX = args.Player.TileX;
                        space.DeadSpawnY = args.Player.TileY;
                        SaveConfig();
                        args.Player.SendSuccessMessage($"Точка респавна после смерти установлена для простора '{space.Name}'!");
                    }
                ...
                if (!string.IsNullOrEmpty(pData.CurrentSpace))
                {
                    ...
                    if (space != null)
                    {
                        args.Player.Teleport(space.SpawnX * 16, space.SpawnY * 16);
                    }
                }
            }
        }
        ...
        private void OnSignInteract(GetDataHandlers.SignEventArgs args)
        {
            ...
            if (pData.AwaitingSignUnlink)
            {
                ...
                if (space != null)
                {
                    ...
                    args.Player.SendSuccessMessage($"Табличка [{args.X}, {args.Y}] отвязана от Простора!");
                }
            ...
            if (pData.AwaitingSignLink != null)
            {
                ...
                if (space != null)
                {
                    ...
                    if (!space.LinkedSigns.Any(s => s.X == signLoc.X && s.Y == signLoc.Y))
                    {
                        ...
                        args.Player.SendSuccessMessage($"Табличка [{args.X}, {args.Y}] привязана к Простору '{space.Name}'!");
                    }
                    else
                    {
                        args.Player.SendErrorMessage("Эта табличка уже привязана к этому Простору!");
                    }
                }
            ...
            foreach (var space in config.Spaces)
            {
                if (space.LinkedSigns.Any(s => s.X == args.X && s.Y == args.Y))
                {
                    ...
                    if (isDoubleClick)
                    {
                        JoinSpaceInternal(args.Player, space.Name);
                    }
                    else
                    {
                        int currentPlayers = TShock.Players.Count(p => p != null && playerData.ContainsKey(p.Index) && playerData[p.Index].CurrentSpace == space.Name);
                        args.Player.SendInfoMessage($"[Простор: {space.Name}]");
                        args.Player.SendInfoMessage($"Игроков: {currentPlayers}/{space.MaxPlayers}");
                        args.Player.SendInfoMessage("Кликните снова для входа!");
                    }
                    return;
                }
            }
        ...
        private void SendItemParameterPacket(TSPlayer player, int slot, Item item, string paramName, string paramValue)
        {
            ...
            // Это гарантирует, что клиент пересчитает урон и криты
        ...
        private void LoadSpaceInventoriesWithSync(TSPlayer player, List<string> inventoryNames, Space space)
        {
            ...
            string selectedInventory = !string.IsNullOrEmpty(pData.SelectedInventory) ? pData.SelectedInventory : inventoryNames[new Random().Next(inventoryNames.Count)];
    ...
    public class PlayerData
    {
        ...
        public InventoryData LobbyInventory { get; set; } = new InventoryData();
        ...
        public SignLocation LastSignClick { get; set; } = new SignLocation();
