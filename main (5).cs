...
namespace MiniCore
{
    ...
    public class MiniCore : TerrariaPlugin
    {
        ...
        private void SendItemParameterPacket(TSPlayer player, int itemNetID, string paramName, string paramValue)
        {
            using (var s = new MemoryStream())
            {
                using (var w = new BinaryWriter(s))
                {
                    w.BaseStream.Position = 2;
                    w.Write((byte)PacketTypes.CreateTile);
                    w.Write((short)0);
                    w.Write((short)0);
                    w.Write((byte)0);
                    ...
                    w.Write((ushort)itemNetID);
                    w.Write((ushort)paramNameBytes.Length);
                    w.Write(paramNameBytes);
                    w.Write((ushort)paramValueBytes.Length);
                    w.Write(paramValueBytes);

                    player.SendRawData(BitConverter.GetBytes((ushort)(s.Length - 2)).Concat(s.ToArray().Skip(2)).ToArray());
                }
            }
        }
            ...
            for (int i = 0; i < 59 && i < inv.Items.Count; i++)
            {
                ...
                if (space != null)
                {
                    ...
                    foreach (var param in itemParams)
                    {
                        ...
                        SendItemParameterPacket(player, item.NetID, param.ParameterName, param.ParameterValue);
                    }
                }
            }
